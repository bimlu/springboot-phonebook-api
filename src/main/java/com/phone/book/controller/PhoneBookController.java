...
//			phoneBookService.saveOrUpdate(otpDetails); 
    ...
    public ResponseEntity<Object> updateNumber(@RequestBody User user) {
    ...
    public ResponseEntity<EditContactResponse> editContacts(@PathVariable("id") int id, @RequestBody Contacts contacts)
            throws Exception {
        try {
            contacts.setId(id);
    ...
    public ResponseEntity<Jsontoken> checkOTP(@RequestBody testBody body) throws Exception {
        ...
        if (otp.getOtp().matches(body.getOtp())) {
            ...
            final UserDetails userDetails = phoneBookServiceImpl
                    .loadUserByUsername(String.valueOf(user.getPhoneNumber()));
